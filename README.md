...
> 把小红书 / 抖音笔记链接，变成可编辑的文字稿。小红书免登录直连 CDN；抖音走 redfox API。图文走 OCR、视频走语音转写（ASR）。
...
- **抖音 / 多平台**：`douyin_getter.py` 基于 redfox.hk API，支持抖音、快手、B站、YouTube 等十多平台（需 API Key）。
...
# 抖音 / 多平台下载需 redfox API Key：export REDFOX_API_KEY=ark_你的key
```
...
│   ├── douyin_getter.py  # 抖音/多平台下载（redfox API，需 Key）
...
- 抖音 / 多平台下载依赖 redfox.hk API，需自备 Key；抖音图文未验证。
